...
namespace Bicep.Core.Diagnostics
{
    public static class DiagnosticBuilder
    {
        ...
        public class DiagnosticBuilderInternal
        {
            ...
            public ErrorDiagnostic MissingDiscriminator(string discriminatorKey, IEnumerable<TypeSymbol> keys) => new ErrorDiagnostic(
                TextSpan,
                "BCP078",
                $"Property '{discriminatorKey}' must be supplied. Valid key types are: {string.Join(", ", keys.Select(k => k.Name))}.");

            public ErrorDiagnostic FailedToMatchDiscriminator(string discriminatorKey, IEnumerable<TypeSymbol> keys) => new ErrorDiagnostic(
                TextSpan,
                "BCP079",
                $"Unable to match a known type for property '{discriminatorKey}'. Valid key types are: {string.Join(", ", keys.Select(k => k.Name))}.");
        }
